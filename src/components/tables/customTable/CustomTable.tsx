...
const CustomTable: React.FC<Props> = (props) => {
  ...
  const [favorites, setFavorites] = useState<{ [key: string]: boolean }>(
    props.bodyData.reduce((acc, item) => {
      acc[item._id] = item.isFavorite;
      return acc;
    }, {} as { [key: string]: boolean })
  );

  const handlePinClick = async (id: string, index: number) => {
    if (!id) {
          ...
          <td className={classes.actions}>
            ...
            <div className={classes.actions__box}>
              <div
                ...
                onClick={() => handlePinClick(item._id, index)}
              >
                <FaStar
                  size={24}
                  color={favorites[item._id] ? "gold" : "gray"}
                />
              </div>
            </div>
          </td>
